...
//    if(rflag==false) return;
  ...
  for( reco::VertexCollection::const_iterator vtx = vertices_->begin(); vtx != vertices_->end(); ++vtx, ++firstGoodVertexIdx){
    ...
    if( !isFake && vtx->ndof()>=4. && vtx->position().Rho()<=2.0 && fabs(vtx->position().Z())<=24.0) {
      firstGoodVertex = vtx;
      nBranches_->PV_isgood.push_back(true);
      N_GoodVertex++;
    }
    else nBranches_->PV_isgood.push_back(false);
...
//      break;
  ...
  nBranches_->PV_N_good=N_GoodVertex; 
...
//    nBranches_->PV_x.push_back(-1);
//    nBranches_->PV_y.push_back(-1);
