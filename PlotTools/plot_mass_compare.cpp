#include "helper.cpp"
void plot_mass_compare(string filenames_1,string filenames_2, string plotname, string legendname_1,string legendname_2, bool isLog = false){
  int nbins=20;
  float min=0.;
  float max=20;
...
//    hs_2->SetMinimum(0.1);
  ...
  pad2->Draw();
  pad1->cd();
  TH1F *hist1 = new TH1F("hist1","Distribution of mass/#tracks",nbins,min,max);
  TH1F *hist2 = new TH1F("hist2","Distribution of mass/#tracks",nbins,min,max);
...
//  hist1->SetLineColor(kBlack);
  ...
  hs_1->Draw("hist");
  hs_1->Draw("ESAME");
  hs_1->GetXaxis()->SetTitle("mass/#Tracks (GeV)");
  ...
  hs_2->Draw("ElpSAME");
  hs_2->GetXaxis()->SetTitle("mass/#Tracks (GeV)");
  ...
  hist_ratio->GetXaxis()->SetTitle("mass/#Tracks (GeV)");
