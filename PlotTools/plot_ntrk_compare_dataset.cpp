#include "helper.cpp"
void plot_ntrk_compare_dataset(string filenames_1,string filenames_2, string plotname, string legendname_1,string legendname_2,string massregion = "inclu", string insta_lumi = "inclu", bool isLog = false){
  int nbins=85;
  float min=0.;
  float max=170.;
...
//  }
  ...
  gStyle->SetOptStat(0);
  THStack *hs_1 = new THStack("hs_1","Distribution of tracks (highPurity PVAssociationQuality>=4)");
  THStack *hs_2 = new THStack("hs_2","Distribution of tracks (highPurity PVAssociationQuality>=4)");
...
//    hs_2->SetMinimum(0.1);
  ...
  if(massregion == "verylow") {selection += "&&Instanton_Trk_TrkCut_mass>20&&Instanton_Trk_TrkCut_mass<=40";nbins=40; max=40;}
  else if (massregion == "low") {selection += "&&Instanton_Trk_TrkCut_mass>40&&Instanton_Trk_TrkCut_mass<=80";nbins=50; max=50;}
  else if (massregion == "medium") {selection += "&&Instanton_Trk_TrkCut_mass>200&&Instanton_Trk_TrkCut_mass<=300";nbins=70; max=140;}
  ...
  TH1F *hist = new TH1F("hist","Distribution of tracks",nbins,min,max);
  ...
  for(int i=0; i<filename_1.size(); i++){
    ...
    if(t->GetEntries(selection.c_str())>0){
      ...
      t->Draw("Instanton_N_Trk_PVAssociationQualityLeq4_highPurity>>hist",selection.c_str(),"goff");
...
//  t_1->Draw("Instanton_N_Trk_PVAssociationQualityLeq4_highPurity>>hist1",selection.c_str());
...
//  t_2->Draw("Instanton_N_Trk_PVAssociationQualityLeq4_highPurity>>hist2",selection.c_str());
...
*/
  ...
  hist_diff->GetXaxis()->SetTitle("# Tracks");
  ...
  pad1->Update();
  c->Update();
  plotname += "_"+massregion+"_ntrk_lumi"+insta_lumi;
