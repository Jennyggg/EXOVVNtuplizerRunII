#include "helper.cpp"
void plot_truePU_comparedataMC(string filenames_1,string filenames_2, string plotname, string legendname_1,string legendname_2, bool isLog = false){
  int nbins=65;
  float min=0;
  float max=65;
...
//    hs_2->SetMinimum(0.1);
  ...
  for(int i=0; i<filename_1.size(); i++){
    ...
    if(t->GetEntries(selection.c_str())>0){
      ...
      t->Draw("nPuVtxTrue>>hist",selection.c_str(),"goff");
  ...
  for(int i=0; i<filename_2.size(); i++){
    ...
    if(t->GetEntries(selection.c_str())>0){
      ...
      t->Draw("Instan_Lumi_per_bunch_mean*69200>>hist",selection.c_str(),"goff");
      hist=(TH1F*)gDirectory->Get("hist");
      hist2->Add(hist);
    }
    t->Reset();
